...
class _State extends State<CheckoutPaymentView> {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse paymnetResponse) async {
    ...
    context.read<CheckoutBloc>().add(OrderStatusChanged(orderStatus: OrderStatus.PaymentSuccess));
  }

  Future<void> _handlePaymentError(PaymentFailureResponse response) async {
    // Do something when payment fails
    print('zzzzzzzzzzzzzzzzzzzzz Fail');
    context.read<CheckoutBloc>().add(OrderStatusChanged(orderStatus: OrderStatus.PaymentFailed));
  }
      ...
      body: LayoutBuilder(builder: (context, constraints) {
       return  BlocListener<CheckoutBloc,CheckoutState>(
         listener: (context,state){
           if(state.orderStatus== OrderStatus.OrderCreationSuccess && state.paymentTypeState==PaymentTypeState.online){
             _checkOut();
           }
         },
                                       ...
                                       Row(
                                         children: [
                                           Radio(
                                               ...
                                               groupValue: state.paymentTypeState,
                                               onChanged: (value) {
                                                 context.read<CheckoutBloc>().add(
                                                     PaymentMethodChanged(
                                                         paymentTypeState: value));
                                               }),
                                           SizedBox(
                                             width: 8,
                                           ),
