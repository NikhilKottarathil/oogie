    ...
    if (event is UpdateProductData) {
      ...
      if (AppData().isUser) {
        if (state.productModel.isInWishList) {
          try {
            await productRepository.deleteProductFromWishlist(wishlistId: state.productModel.wishListId );
          ...
          try {
           String wishlistId= await productRepository.addProductToWishList(productId: state.productModel.id);
